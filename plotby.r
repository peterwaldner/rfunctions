...
#                see legend()
#   sortlvar   Ordering of lvar, e.g. 1, -1, 0
#
...
#   bty        "n" no legend box, "c" legend box
#
# Explanation
   ...
   ...)
{
  ...
  if (byvar == "") 
  {
    ...
    {
      # sort data
      if (is.numeric(d[,lvar])) {
        d = d[order(sortlvar*d[,lvar],d[,xvar]),]
      } else {
    ...
    # axis range
    xlims = range(d[,xvar], na.rm=TRUE,finite=TRUE)
    ylims = range(d[,yvars], na.rm=TRUE,finite=TRUE)
    ...
    if (!lvar == "" | !length(yvars) == 1 )
    {
      ...
      {
        legend(lpos, litems, col=colorlist[1:length(litems)], lty=1, lwd=4, ncol=ncol,bty=bty)
      }
      if (!(paste(ltitle,lvar) == " "))
      {
        legend(lpos, litems, title=paste(ltitle, lvar), col=colorlist[1:length(litems)], lty=1, lwd=4, ncol=ncol,bty=bty)
      }
    }
    ...
    # axis range
    xlims = range(d[,xvar], na.rm=TRUE, finite=TRUE)
    # yvars separately, because finite does not work in multicolumn dataframe
    ylims = range(d[,yvars[1]], na.rm=TRUE, finite=TRUE)
    for (yvar in yvars) {
         ymin=min(d[,yvar], na.rm=TRUE, finite=TRUE);
         ymax=max(d[,yvar], na.rm=TRUE, finite=TRUE);
         ylims=c(min(ymin,ylims[1], na.rm=TRUE, finite=TRUE),
                 max(ymax,ylims[2], na.rm=TRUE, finite=TRUE));
    }      
  ...
  if (!(byvar == ""))
  {
    ...
    else
    {
      ...
      if (is.numeric(d[,lvar])) {
        d = d[order(d[,byvar],sortlvar*d[,lvar],d[,xvar]),]
      } else {
