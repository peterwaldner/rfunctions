...
sufixer <- function(d,byvars,sid,sfelder,idvars=c(),ssort=c(),sep="_",
                    stringsAsFactors=FALSE) 
{
   ...
   {  
      if (length(idvars)>0 & calculations == 0) {
         ...
         names(d1) = c(byvars,idvars,paste(sfelder,sidval,sep=sep));
      } else {
         d1 = d[d[,sid]==sidval,c(byvars,sfelder)];
         names(d1) = c(byvars,paste(sfelder,sidval,sep=sep));
      }
      if (calculations==0) {
         d2=d1
         calculations=1
      } else {
         d2=merge(d2,d1,by=byvars,all=TRUE,stringsAsFactors=stringAsFactors)
      }
   }
